...
namespace fw3\io\rdbms\query_builder\drivers\general\functions\math;
    ...
    FunctionInterface
{
     ...
     */
    public static function factory(...$arguments): AbsFunction
    {
        $instance   = new static();
        if (!empty($arguments)) {
    ...
    public function build(): BuildResultInterface
    {
        ...
        if ($this->value instanceof Buildable) {
            ...
            if ($this->doUsePlaceholder()) {
                ...
                $clause_format  = strpos((string) $this->value, '.') ? 'abs(%f)' : 'abs(%d)';
